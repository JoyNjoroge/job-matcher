# ApplyBotPro Browser Extension

Smart job application assistant that auto-fills forms using AI.

## 🚀 Features

- **Auto-Fill Job Applications**: Uses Gemini AI to intelligently fill out job application forms
- **Profile Sync**: Automatically syncs with user's ApplyBotPro account
- **Job Tracking**: Tracks applications and syncs back to ApplyBotPro
...
4. Select the `applybotpro-extension` folder
...
On your ApplyBotPro website, add a button/section for users to install the extension:
...
### API Key Security
**IMPORTANT**: Never commit your Gemini API key to version control!

For production:
1. Store the API key in environment variables
2. Make API calls from your backend instead of the extension
3. Have the extension call your backend, which then calls Gemini
...
app.post('/api/extension/fill-form', async (req, res) => {
  ...
  // Call Gemini API from server (API key is secure)
  const suggestions = await callGeminiAPI(fields, userProfile);
  
  res.json({ suggestions });
});
```

Then update `content.js` to call your backend instead of Gemini directly.
...
2. Check if job appears in Applications table on ApplyBotPro
